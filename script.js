...
function generateCSV(portal, flights) {
    ...
    if (portal === 'travclan') {
        ...
        rows = flights.map(f => [
            f.portal, f.flightNumber, `"${f.airline}"`, f.origin, f.destination, 
            `${f.origin}-${f.destination}`,
            `"${f.departureTime}"`, `"${f.arrivalTime}"`, f.duration,
            f.stops === 0 ? 'Direct' : `${f.stops} stop${f.stops > 1 ? 's' : ''}`,
            f.baseFare, f.finalFare, `"${f.fareClass}"`, `"${f.fareIdentifier}"`,
            `"${f.provider}"`, `"${f.baggage}"`, `"${f.cabinBaggage}"`
        ]);
    } else if (portal === 'tripjack') {
        headers = ['Portal', 'Flight Number', 'Airline', 'Origin', 'Destination', 'Sector',
            'Departure Time', 'Arrival Time', 'Duration', 'Direct/Indirect',
            'Base Fare', 'Final Fare', 'Net-TDS', 'Fare Type',
            'Net Fare', 'Baggage', 'Cabin Baggage'];
        
        rows = flights.map(f => [
            f.portal, `"${f.flightNumber}"`, `"${f.airline}"`, f.origin, f.destination,
            `${f.origin}-${f.destination}`,
            `"${f.departureTime}"`, `"${f.arrivalTime}"`, f.duration, f.directIndirect,
            f.baseFare, f.finalFare, (f.netMinusTds || 0).toFixed(2), `"${f.fareType}"`,
            `"${f.provider}"`, `"${f.baggage}"`, `"${f.cabinBaggage}"`
        ]);
    } else {
        headers = ['Portal', 'Flight Number', 'Airline', 'Origin', 'Destination', 'Sector',
        ...
        rows = flights.map(f => [
            ...
            `${f.origin}-${f.destination}`,
            `"${f.departureTime}"`, `"${f.arrivalTime}"`, f.duration,
            f.directIndirect || (f.stops === 0 || f.stops === 'Non-Stop' ? 'Direct' : 'Indirect'),
            f.baseFare, f.finalFare, `"${f.fareType}"`, `"${f.provider}"`,
            `"${f.baggage}"`, `"${f.cabinBaggage}"`, `"${f.cabinClass || 'Economy'}"`
        ]);
    }
